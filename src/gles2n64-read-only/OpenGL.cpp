...
bool OGL_Start()
{
    ...
    /* Set the video mode */
    fprintf(stderr,  "[gles2n64]: (II) Setting video mode %dx%d...\n", (int)OGL.width, (int)OGL.height );
    if (!(OGL.hScreen = SDL_SetVideoMode( OGL.width, OGL.height /*- 32*/, 32, SDL_OPENGL | SDL_FULLSCREEN | SDL_NOFRAME )))
    {
        printf( "[gles2n64]: (EE) Error setting videomode %dx%d: %s\n", (int)OGL.width, (int)OGL.height, SDL_GetError() );
        SDL_QuitSubSystem( SDL_INIT_VIDEO );
        return FALSE;
    }
    fprintf(stderr, " 7 \n");
