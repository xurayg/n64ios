...
#else
  sprintf(buffer, "%s", "/Applications/n64ios.app/");
  return true;
#endif
}
#endif
...
#if defined(__APPLE__) && defined(__arm__)
  strncpy(l_ConfigDir, "/Applications/n64ios.app/config/", PATH_MAX);
  strncpy(l_InstallDir, "/Applications/n64ios.app/", PATH_MAX);
  return;
